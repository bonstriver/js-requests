...
const problemEight = () => {
    axios.get('http://localhost:3000/query-test?myQuery=42').then((res)=>{
        console.log(res.data)
    })
}

document.getElementById('query-button').addEventListener('click', problemEight)
...
const createFood = (event) => {
    ...
    axios.post('http://localhost:3000/food', body).then((res)=>{
        ...
        line.textContent = res.data[res.data.length-1]
